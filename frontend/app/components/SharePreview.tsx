...
export default function SharePreview({ previewUrl, shareUrl }: SharePreviewProps) {
  ...
  const handleWhatsAppShare = () => {
    const message = encodeURIComponent(`Check out my Wallfeel room preview: ${urlToShare}`)
    const whatsappUrl = `https://wa.me/?text=${message}`
    window.open(whatsappUrl, '_blank', 'noopener,noreferrer')
  }

  const handleEmailShare = () => {
    const subject = encodeURIComponent('My Wallfeel Room Preview')
    const body = encodeURIComponent(`Check out my Wallfeel room preview: ${urlToShare}`)
  ...
  const handleNativeShare = async () => {
    ...
    try {
      ...
      await navigator.share({
        title: 'My Wallfeel Room Preview',
        text: 'Check out my Wallfeel room preview',
        url: urlToShare,
      })
    } catch (error) {
    ...
    <div className="card" style={{ marginTop: '24px', padding: '24px' }}>
      ...
      <div className="flex flex-wrap justify-center gap-3">
        ...
        <button
          ...
          style={{
            background: copySuccess ? 'var(--gold)' : 'var(--bg-secondary)',
